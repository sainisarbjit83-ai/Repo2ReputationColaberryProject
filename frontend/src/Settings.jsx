  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      ...
      <div className="flex items-center justify-between px-6 py-3 bg-white border border-gray-200 rounded-3xl shadow-sm mb-4">
        ...
        <div className="flex items-center gap-3">
          <a href="/" className="text-sm font-medium text-gray-600 hover:text-indigo-600 transition">
            ← Back to repos
          </a>
          <button
            onClick={onLogout}
      ...
      <div className="max-w-2xl mx-auto bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        <h2 className="text-lg font-bold text-gray-900 mb-1">Connected GitHub Accounts</h2>
        <p className="text-sm text-gray-500 mb-6">
          Repos from all connected accounts appear in Browse. The primary account is used for login.
        </p>

        {successMsg && (
          <div className="mb-4 flex items-center justify-between px-4 py-3 rounded-xl border border-green-200 bg-green-50 text-green-700 text-sm font-medium">
        ...
        {error && (
          <div className="mb-4 flex items-center justify-between px-4 py-3 rounded-xl border border-red-200 bg-red-50 text-red-700 text-sm">
            <span>{error}</span>
        ...
        {loading ? (
          <p className="text-gray-400 text-sm py-4">Loading accounts…</p>
        ) : (
          <div className="space-y-3 mb-6">
            {accounts.length === 0 ? (
              <p className="text-gray-400 text-sm py-4">No GitHub accounts connected.</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {account.github_email && (
                      <p className="text-xs text-gray-400">{account.github_email}</p>
                    )}
                  </div>
                  {account.is_primary && (
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gray-900 hover:bg-gray-800 text-white text-sm font-semibold transition"
        >
        ...
        <p className="mt-4 text-xs text-gray-400">
          Tip: GitHub may keep you logged in. To connect a different account, first{' '}
          <a href="https://github.com/logout" target="_blank" rel="noopener noreferrer" className="underline hover:text-gray-600">
            sign out of GitHub
          </a>{' '}
